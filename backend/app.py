...
app.config['EXPORT_FOLDER'] = os.path.join(APP_ROOT, '..', 'exports')
...
def get_subjects():
    try:
        subjects = execute_query("""\n            SELECT s.*, \n                   GROUP_CONCAT(DISTINCT CONCAT(p.name, ' Y', sec.year, '-', sec.section_name) \n                                SEPARATOR ', ') as assigned_classes\n            FROM subjects s\n            LEFT JOIN subject_sections ss ON s.subject_id = ss.subject_id\n            LEFT JOIN sections sec ON ss.section_id = sec.section_id\n            LEFT JOIN programs p ON sec.program_id = p.program_id\n            GROUP BY s.subject_id\n            ORDER BY s.name\n        """, fetch=True)
...
def get_sections():
    ...
    - for_role=teacher  -> only sections taught by the logged-in teacher
    - for_role=student  -> only the logged-in student's own section

    If no filter is provided, all sections are returned.
    """
    try:
        role_filter = request.args.get('for_role')
        user_id = session.get('user_id')

        if role_filter == 'teacher' and user_id:
            # Sections that have subject assignments for this teacher
            sections = execute_query(
                """
                SELECT DISTINCT s.*, p.name as program_name
                FROM sections s
                JOIN programs p ON s.program_id = p.program_id
                JOIN subject_assignments sa ON sa.section_id = s.section_id
        ...
        elif role_filter == 'student' and user_id:
            # The logged-in student's own section (if any)
            sections = execute_query(
                """
            ...
            # Default: all sections (original behavior)
            sections = execute_query(
                """
                SELECT s.*, p.name as program_name
                FROM sections s
...
def generate_timetable():
    try:
        from test_generator import AdvancedTimetableGenerator
        ...
        if result.get('success'):
            ...
            if generator.save_timetable():
                return jsonify({
                    'message': 'Timetable generated and saved successfully with advanced algorithms',
                    'timetable': result['timetable']
                }), 200
            else:
                return jsonify({'error': 'Timetable generated but failed to save'}), 500
        else:
    ...
    try:
        ...
        for timeslot in all_timeslots:
            ...
            if is_break:
                ...
                if section_id:
                    # Filter by section - show empty slots if no entry
                    key = (timeslot['timeslot_id'], int(section_id))
                    entry = scheduled_map.get(key)
                    if entry:
                        formatted[day].append({
                            'timeslot': slot_key,
                        ...
                        # Empty slot for this section - show structure
                        formatted[day].append({
                            'timeslot': slot_key,
                            'is_empty': True
                        })
                else:
                    # No section selected - show only structure (empty slots)
                    # Don't show any scheduled entries, just the structure
                    formatted[day].append({
                        'timeslot': slot_key,
                        'is_empty': True
                    })
...
def export_timetable(format_type):
    try:
        ...
        manager = ExportManager(export_dir=app.config['EXPORT_FOLDER'])
        
        if format_type.lower() == 'csv':
            filepath = manager.export_to_csv(section_id, teacher_id, room_id, version_id, user_id)
        elif format_type.lower() in ['excel', 'xlsx']:
            filepath = manager.export_to_excel(section_id, teacher_id, room_id, version_id, user_id)
        elif format_type.lower() in ['pdf']:
            filepath = manager.export_to_pdf(section_id, teacher_id, room_id, version_id, user_id)
        elif format_type.lower() in ['ical', 'ics']:
            filepath = manager.export_to_ical(section_id, teacher_id, room_id, version_id, user_id)
        else:
...
@app.route('/api/export-multiple/<format_type>', methods=['GET'])
@role_required('coordinator', 'admin', 'teacher', 'student')
def export_multiple_timetables(format_type):
    """
    Export multiple section timetables at once into a single file.

    Currently supported formats: Excel (xlsx).
    Frontend should pass a comma-separated list of section IDs via the
    "section_ids" query parameter, e.g. ?section_ids=1,2,3
    """
    try:
        from .export_manager import ExportManager

        section_ids_param = request.args.get('section_ids', '').strip()
        if not section_ids_param:
            return jsonify({'error': 'section_ids query parameter is required'}), 400

        try:
            section_ids = [int(s) for s in section_ids_param.split(',') if s.strip()]
        except ValueError:
            return jsonify({'error': 'Invalid section_ids parameter'}), 400
        ...
        if format_lower in ['excel', 'xlsx']:
            filepath = manager.export_multiple_sections_to_excel(section_ids, user_id)
        else:
            return jsonify({'error': 'Multiple export is currently supported only for Excel format'}), 400
...
def get_bulk_template(template_type):
    try:
        ...
        bulk_ops = BulkOperations()
        filepath = bulk_ops.export_template(template_type, export_dir=app.config['EXPORT_FOLDER'])
